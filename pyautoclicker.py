...
- **Recorder**: press **Start Recording**, then **hold CTRL** while you click to add points; release CTRL to finish.
- **Auto-save prompt** appears after recording (can be toggled in Settings).
- **Save** never overwrites: unique filenames are created automatically.
- **Sequence Manager** lets you browse, load, and delete saved sequences.
- **Hotkeys** accept single keys (X, P, 1), named keys (Space, Enter, Esc, arrows, etc.), F-keys, and combos.
"""
import os, sys, time, threading, random, json, configparser, platform, re as _re, string
...
ICON_NAME = "clicker.ico"
ICON_PNG = "clicker.png"
...
def normalize_hotkey(s: str) -> str:
    if not s: return ""
    parts = [p.strip().lower() for p in _re.split(r"[+\-]", s) if p.strip()]
    if not parts: return ""
...
class App:
    ...
    def show_settings_window(self):
        ...
        help_txt = (
            "Recording:\n"
            " • Click 'Start Recording', then hold CTRL and click to add points. Release CTRL to stop. The Save dialog opens automatically (toggle in Settings).\n"
            " • Saving never overwrites. A unique filename is created if the chosen name exists.\n\n"
            "Playback:\n"
            " • Start/Stop (default F6), Pause/Resume (default F9). If no sequence is loaded, clicks at the cursor using Base interval (ms).\n"
            " • In Save dialog: 'Inter‑click delay (ms)' and 'Repeats' control the delay between each step and how many loops to run (0 = infinite).\n\n"
            "Hotkeys:\n"
            " • Single keys like X, P, or 1; named keys (Space, Enter, Esc, Tab, arrows, Home/End, PageUp/Down, Insert, Delete, Backspace); F‑keys; combos like Ctrl+Alt+S.\n"
            " • If a single key interferes with typing, use a combo.\n\n"
            "Bubble:\n"
            " • Drag anywhere on the bubble to move it. The top line shows the loaded profile name.\n"
        )
    ...
    def show_bubble(self):
        ...
        self._drag = {"x":0, "y":0}
        def start_drag(e): self._drag.update(x=e.x_root - w.winfo_x(), y=e.y_root - w.winfo_y())
        def do_drag(e): w.geometry(f"+{int(e.x_root - self._drag['x'])}+{int(e.y_root - self._drag['y'])}")
        for widget in (w, frm, self.lbl_status, self.lbl_profile):
            widget.bind("<Button-1>", start_drag); widget.bind("<B1-Motion>", do_drag)

        self.root.update_idletasks(); w.geometry(f"+{self.root.winfo_screenwidth()-240}+40")
        self._bubble_update()
